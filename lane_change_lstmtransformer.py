...
# LSTM과 Transformer를 결합한 모델 설정
class LSTMTransformerModel(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_layers_lstm, num_classes=2):
        super(LSTMTransformerModel, self).__init__()
        # LSTM Layer
        self.lstm = nn.LSTM(input_dim, hidden_dim, num_layers=num_layers_lstm, batch_first=True)
        # Transformer Layer
        self.encoder_layer = nn.TransformerEncoderLayer(d_model=hidden_dim, nhead=4, dim_feedforward=512, dropout=0.1)
    ...
    # 모델의 forward 함수 수정
    def forward(self, x):
        # LSTM expects 3D input (batch_size, sequence_length, feature_dim)
        if len(x.shape) == 2:  # If the input is 2D (batch_size, feature_dim), reshape it to (batch_size, 1, feature_dim)
        ...
        lstm_out = lstm_out.permute(1, 0, 2)  # Rearrange for Transformer input
        
        # Transformer Encoder
        transformer_out = self.transformer_encoder(lstm_out)
        
        # Final output through fully connected layer
        transformer_out = transformer_out.permute(1, 0, 2)  # Back to (batch_size, sequence_length, hidden_dim)
        final_output = self.fc(transformer_out[:, -1, :])  # Use the last output for classification
...
# 모델 학습 함수
def train_model(df_grouped, label_col, hidden_dim, num_layers_lstm, dropout, lr, input_dim=5, epochs=50):
    dataset = LaneChangeDataset(df_grouped, label_col)
    dataloader = DataLoader(dataset, batch_size=32, shuffle=True)

    model = LSTMTransformerModel(input_dim=input_dim, hidden_dim=hidden_dim, num_layers_lstm=num_layers_lstm)
...
def objective(trial):
    ...
    num_layers_lstm = trial.suggest_int('num_layers_lstm', 1, 3)
    ...
    # 모델 학습 및 평가
    model = train_model(train_df_case_1, 'label_1', hidden_dim, num_layers_lstm, dropout, lr)
...
for case, train_df, test_df, label_col in [(1, train_df_case_1, test_df_case_1, 'label_1'), 
    ...
    if len(train_df) == 0:
        ...
        # 모델 학습
        model = train_model(train_df, label_col, trial.params['hidden_dim'], trial.params['num_layers_lstm'], trial.params['dropout'], trial.params['lr'])
